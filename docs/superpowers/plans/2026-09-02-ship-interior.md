...
- [ ] **0.3 Bare hold with a hole.** Temporary deck: hold shell boxes and a visual ceiling with a 1.8 m round opening at the cupola position, an unlit grey box interior, one point light. Camera on a fixed rail (`?pose=rail&t=0..1`) below the hole. Check: screenshots at t = 0, 0.5, 1 show the real Earth through the hole with visible parallax (from the rail only sky is geometrically reachable through the well); a fourth shot from `?pose=cupola` shows the ship's own dorsal deck with space behind it.
