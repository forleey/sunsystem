...
- [ ] **0.3 Bare hold with a hole.** Temporary deck: hold shell boxes and a visual ceiling with a 1.8 m round opening at the cupola position, an unlit grey box interior, one point light. Camera on a fixed rail (`?pose=rail&t=0..1`) below the hole. Check: screenshots at t = 0, 0.5, 1 show the real Earth and the engine humps through the hole with visible parallax.
- [ ] **0.4 `hull_glass.js` cupola.** Km-scale seven-pane cupola on `shipView.grp` at the cupola position (outside the GLB subtree), warm emissive disc, layer bit. Check: chase-cam screenshot shows the bubble on the hull at the saddle between the humps; boarded screenshot does not show it.
