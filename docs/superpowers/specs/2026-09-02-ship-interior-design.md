...
|---|---|---|
| **M0 Spike: the window is real** | `rig.js`, `hull_frame.js`, the second pass, focus lock, a bare box hold with a round hole in the ceiling at the cupola position, `hull_glass.js` cupola outside, `V` to board and leave. No dressing, no walk (camera on a fixed rail). | Screenshots show the real Earth through the hole with parallax along the rail, and the ship's own dorsal deck from the cupola pose; `WINDOWTEST=PASS`; `perf()` within budget. **Nothing else starts before this passes.** |
